...
def main():
    ...
    for bribe in bribes:
        if (
            not bribe['active'] 
            or voter in bribe['blacklistedAddresses']
            or int(bribe['remainingWeeks']) <= 0
        ): #bribe['closed'] or (next_period <= bribe['endTimestamp'])
            continue
    ...
    df.to_csv(f'data/gauges-{date}.csv', index=False)
    assert False
